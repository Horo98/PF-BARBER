  ...
  const { logIn} = useContext(CartContext)
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            console.log(values)
            await axios.post('https://pf-barber-production-4b93.up.railway.app/auth/login', values).then(cred=> document.cookie = `token=${cred.data.token}; max-age=${500*500}; path=/; samesite=strict`
            ).catch((error)=> {
                return (Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
...
<br />
                        <div>
            <a class="btn btn-primary btn-sm" href={`/auth/google/signup`}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-google" viewBox="0 0 16 16">
